...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # 1
        word_embeddings = self.model_embeddings.source(source_padded)
        X = nn.utils.rnn.pack_padded_sequence(word_embeddings, source_lengths)

        # 2
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens_padded, _ = nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        enc_hiddens_permuted = enc_hiddens_padded.permute(1, 0, 2)
        last_hidden_cat = torch.cat(tensors=(last_hidden[0], last_hidden[1]), dim=1)
        # 3
        init_decoder_hidden = self.h_projection(last_hidden_cat)
        last_cell_cat = torch.cat(tensors=(last_cell[0], last_cell[1]), dim=1)
        ...
        return enc_hiddens_permuted, dec_init_state
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            dec_state, o_prev = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)[0:2]
            combined_outputs.append(o_prev)
            
        # 4
        stacked_outputs = torch.stack(combined_outputs)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #3
        dec_hidden_unsqueezed = torch.unsqueeze(dec_hidden, dim=1)
        e_t_unsqueezed = torch.bmm(dec_hidden_unsqueezed, enc_hiddens_proj.permute(0, 2, 1))
        e_t = torch.squeeze(e_t_unsqueezed, dim=1)
        ...
        a_t_unsqueezed = torch.bmm(alpha_t_unsqueezed, enc_hiddens)
        a_t = torch.squeeze(a_t_unsqueezed, dim=1)
        #3
